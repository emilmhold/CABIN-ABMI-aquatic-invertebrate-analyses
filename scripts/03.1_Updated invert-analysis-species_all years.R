...
invert.counts <- adj.count %>%
    ...
    mutate(row_total = sum(c_across(5:262), na.rm = TRUE)) %>%
    ungroup() %>%
    group_by(Protocol) %>%
...
png(filename = paste0("figures/invertebrate-protocol-analyses/species-accumulation-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 1200,
    res = 300)
...
hist(subset(site.richness, Protocol == "ABMI")$Simpson)
hist(subset(site.richness, Protocol == "ABMI")$Shannon)
hist(subset(site.richness, Protocol == "ABMI")$Richness)
hist(subset(site.richness, Protocol == "CABIN")$Simpson)
hist(subset(site.richness, Protocol == "CABIN")$Shannon)
hist(subset(site.richness, Protocol == "CABIN")$Richness)
...
png(filename = paste0("figures/invertebrate-protocol-analyses/simpson-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 1200,
    res = 300)
...
png(filename = paste0("figures/invertebrate-protocol-analyses/richness-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 1200,
    res = 300)
...
png(filename = paste0("figures/invertebrate-protocol-analyses/shannon-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 1200,
    res = 300)
...
write_csv(table8, "tables/wilcox analyses for species detections by protocol.csv")

#### Table 9: Model species responses to abiotic data ####
...
overlap.df <- data.frame(Variable = character(),
                         Species = character(),
                         stringsAsFactors = FALSE)
...
for (var in variables) {
    for (spp.id in names(all.coef)) {
        ...
        abmi.int <- c(abmi.row$Estimate - abmi.row$StdErr,
                      abmi.row$Estimate + abmi.row$StdErr)
        cabin.int <- c(cabin.row$Estimate - cabin.row$StdErr,
                       cabin.row$Estimate + cabin.row$StdErr)

        # If intervals overlap, record it
        if (!(abmi.int[2] < cabin.int[1] || cabin.int[2] < abmi.int[1])) {
            overlap.df <- rbind(overlap.df, data.frame(Variable = var,
                                                       Species = spp.id))
        }
    }
}
...
# View summary
overlap.count <- overlap.df %>%
    group_by(Variable) %>%
    summarise(NumSpeciesOverlap = n())

##write overlap table
write.csv(overlap.df, "tables/species with overlapping model coefficients.csv", row.names = FALSE)

##write count table
write.csv(overlap.count, "tables/overlap_counts_per_variable.csv", row.names = FALSE)
...
write.csv(data.frame(SS = pt.test$ss,
                     ...
                     Significance = pt.test$signif), file = paste0("output/procrustes-analysis-", data.type, "-species-level_", Sys.Date(), ".csv"), row.names = FALSE) # No difference in their dispersion
