...
adj.count <- read_excel("output/CABIN Ambiguous parent resolved all taxa_APTC.abundant.xlsx") %>%
    ...
    filter(Year != "2018") #to look at 2022/2023 data
...
png(filename = paste0("figures/invertebrate-protocol-analyses/2022-and-2023-data/species-accumulation-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 1200,
    res = 300)
...
print(ncol(cabin.spp)-4) #184 species

abmi.spp <- data.in %>%
    filter(Protocol == "ABMI") %>%
    select(1:4, where(~ !(is.numeric(.) && sum(., na.rm = TRUE) == 0))) #drop columns where no individuals are observed.
print(ncol(abmi.spp) - 4) #191 species
#
...
png(filename = paste0("figures/invertebrate-protocol-analyses/2022-and-2023-data/simpson-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 1200,
    res = 300)
...
png(filename = paste0("figures/invertebrate-protocol-analyses/2022-and-2023-data/richness-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 1200,
    res = 300)
...
png(filename = paste0("figures/invertebrate-protocol-analyses/2022-and-2023-data/shannon-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 1200,
    res = 300)
...
write.csv(summary(simper.results)$ABMI_CABIN,
          file = paste0("output/simper-analysis-", data.type, "2022-2023-species-level_", Sys.Date(), ".csv"), row.names = TRUE)
...
png(filename = "figures/invertebrate-protocol-analyses/2022-and-2023-data/nmds-adjusted-counts-species-level_2025-09-19.png",
    width = 2400,
    height = 2400,
    res = 300)
...
png(filename = paste0("figures/invertebrate-protocol-analyses/2022-and-2023-data/stressplot-best-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 2400,
    res = 300)
...
write.csv(data.frame(SS = pt.test$ss,
                     ...
                     Significance = pt.test$signif), file = paste0("tables/procrustes-analysis-", data.type, "-2022-and-2023-species-level_", Sys.Date(), ".csv"), row.names = FALSE) # No difference in their dispersion
...
png(filename = paste0("figures/invertebrate-protocol-analyses/2022-and-2023-data/procrustes-", data.type, "-species-level_", Sys.Date(), ".png"),
    width = 2400,
    height = 2400,
    res = 300)
...
print(ggplot() +
          ...
          xlab("Axis 1") +
          ylab("Axis 2") +
          ggtitle("2022 & 2023 adjusted species data") +
          coord_equal() +
          theme_bw())
